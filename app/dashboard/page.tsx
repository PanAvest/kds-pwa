...
function pickEbook(
  e: EbookRow | EbookRow[] | null | undefined
): EbookRow | null {
  if (!e) return null;
...
async function downloadCertPdf(options: DownloadCertOptions) {
  ...
  const filename = certNumber
    ? `PanAvest-Certificate-${certNumber}.pdf`
    : "PanAvest-Certificate.pdf";
  const origin =
    typeof window !== "undefined" && window.location
      ? window.location.origin
      : "https://kdslearning.com";
  ...
  try {
    ...
    // Header gradient
    const grad =
      typeof window !== "undefined" ? makeGradientDataUrl(800, 80) : null;
    if (grad) doc.addImage(grad, "PNG", 0, 0, pageWidth, 22);

    // Logo
    const logoUrl = `${origin}/logo.png`;
    const logoData =
      typeof window !== "undefined" ? await toDataUrl(logoUrl) : null;
    if (logoData) doc.addImage(logoData, "PNG", margin, 8, 24, 12);

    doc.setFontSize(18);
    doc.setTextColor(10, 17, 86);
    doc.text("Knowledge Development Series", pageWidth / 2, 20, {
      align: "center",
    });

    doc.setTextColor(60, 60, 60);
    doc.setFontSize(12);
    doc.text("Certificate of Completion", pageWidth / 2, 30, {
      align: "center",
    });
    ...
    doc.text("has successfully completed", pageWidth / 2, 62, {
      align: "center",
    });
    ...
    doc.setFontSize(11);
    doc.text(
      "For successfully completing the prescribed curriculum and assessments.",
      pageWidth / 2,
      82,
      {
        align: "center",
    ...
    if (typeof cpdPoints === "number")
      doc.text(`CPD Points: ${cpdPoints}`, margin, 116);
    if (typeof scorePct === "number")
      doc.text(`Score: ${scorePct}%`, margin, 124);

    // Signature placeholder
    doc.text("Authorized Signatory", margin, pageHeight - 40);
    doc.text("Prof. Douglas Boateng", margin, pageHeight - 34);
    doc.text(
      "D.Prof., FCILT, FIoD, FIOM, FAC, FIAM, FCIPS, FIC",
      margin,
      pageHeight - 30
    );
    ...
    if (verifyUrl) {
      ...
      const qrData =
        typeof window !== "undefined" ? await toDataUrl(qrUrl) : null;
      if (qrData) {
        const size = 40;
        doc.addImage(
          qrData,
        ...
        doc.text(
          "Scan to verify",
          pageWidth - margin - size / 2,
          pageHeight - margin - size - 8,
          { align: "center" }
        );
      }
    }
...
export default function DashboardPage() {
    ...
    (async () => {
      try {
        ...
        const { data: enrData } = await supabase
          .from("enrollments")
          .select(
            "course_id, progress_pct, courses!inner(id,title,slug,img,cpd_points)"
          )
          .eq("user_id", userId)
        ...
        if (allCourseIds.length > 0) {
          ...
          if (allChapterIds.length > 0) {
            ...
            (slRows ?? []).forEach((s: { id: string; chapter_id: string }) => {
              ...
              if (cid)
                totalSlidesByCourse[cid] = (totalSlidesByCourse[cid] ?? 0) + 1;
            });
          }
          ...
          (progRows ?? []).forEach(
            (r: { course_id: string; slide_id: string }) => {
        ...
        const { data: quizRows } = await supabase
          ...
          .select(
            "course_id, chapter_id, total_count, correct_count, score_pct, completed_at"
          )
          .eq("user_id", userId);
        ...
        const chapterIds = Array.from(
          new Set(attempts.map((a) => a.chapter_id))
        ).filter(Boolean);
        if (chapterIds.length > 0) {
          ...
          (chapterRows as unknown as ChapterInfo[] | null | undefined)?.forEach(
            (row) => {
              map[row.id] = {
                id: row.id,
                title: row.title,
        ...
        try {
          const { data: certData, error: certErr } = await supabase
            .from("certificates")
            .select(
              "id,user_id,course_id,attempt_id,certificate_no,issued_at,courses:course_id(id,title,slug,img,cpd_points)"
            )
            .eq("user_id", userId)
        ...
        const attemptIds = Array.from(
          new Set(certRows.map((c) => c.attempt_id).filter(Boolean) as string[])
        );
        ...
        if (attemptIds.length > 0) {
          const { data: attRows } = await supabase
            ...
            .in("id", attemptIds);
          (attRows as unknown as AttemptRow[] | null | undefined)?.forEach(
            (a) => {
              attemptMap[a.id] = a;
            }
          );
        ...
        const missingCourseIds = certRows
          ...
          .filter(
            (cid) =>
              !metaTmp[cid] &&
              !pickCourse(
                certRows.find((row) => row.course_id === cid)?.courses
              )
          );
        ...
        const certViews: CertificateView[] = certRows.map((c) => {
          const meta = pickCourse(c.courses) ||
            metaTmp[c.course_id] ||
            hydratedMeta[c.course_id] || {
              title: "Course",
              slug: "",
              img: null,
              cpd_points: null,
          ...
          const certNumber =
            (c.certificate_no || "").trim() ||
            makeKdsCertId(userId, c.course_id);
          const attempt = c.attempt_id ? attemptMap[c.attempt_id] : undefined;
          const issuedAt =
            c.issued_at || attempt?.created_at || new Date().toISOString();
          const verifyUrl = `${origin}/verify?cert_id=${encodeURIComponent(
            c.id
          )}`;
          return {
            id: c.id,
        ...
        const completedCourses = Object.entries(progressByCourse)
          .filter(([, pct]) => pct >= 100)
          .map(([cid]) => cid);
        const coursesWithoutCert = completedCourses.filter(
          (cid) => !certRows.some((c) => c.course_id === cid)
        );
        ...
        if (coursesWithoutCert.length > 0) {
          ...
          const examIds = Array.from(
            new Set((examRows ?? []).map((e: any) => e.id).filter(Boolean))
          );
          ...
          (examRows as ExamRow[] | null | undefined)?.forEach((exam) => {
            ...
            const passed =
              (!!attempt.passed || (attempt.score ?? 0) >= passMark) &&
              (attempt.score ?? 0) >= passMark;
            if (!passed) return;
            const meta =
              metaTmp[exam.course_id] || hydratedMeta[exam.course_id];
            provisional.push({
  ...
  const quizByCourse = useMemo(() => {
    const grouped: Record<
      string,
      { attempt: QuizAttempt; chapter: ChapterInfo }[]
    > = {};
    for (const a of quiz) {
    ...
    for (const k of Object.keys(grouped)) {
      grouped[k].sort(
        (l, r) => (l.chapter.order_index ?? 0) - (r.chapter.order_index ?? 0)
      );
    }
    return grouped;
  }, [quiz, chaptersById]);
  ...
  return (
    <div className="min-h-[100svh] bg-[color:var(--color-light)]/40">
        ...
        <div className="fixed inset-0 z-[80] bg-black/40 flex items-end md:items-center md:justify-center">
          <div className="w-full md:w-[520px] rounded-t-2xl md:rounded-2xl bg-white shadow-lg p-4">
            ...
            <p className="text-xs text-muted">
              Your certificates use this name.
            </p>
      ...
      <main
        className={`mx-auto max-w-screen-md ${native ? "pb-2" : "pb-[88px]"}`}
      >
        {/* Quick Actions */}
        <div className="px-4 pt-4 grid grid-cols-3 gap-3">
          <Link
            href="/courses"
            className="rounded-xl bg-white border border-light p-3 text-center text-sm font-medium"
          >
            Explore
          </Link>
          <Link
        ...
        <Section title="Continue learning">
          {loading ? (
            <div className="px-4">
              <div className="h-28 rounded-xl bg-white border border-light animate-pulse" />
            </div>
          ) : enrolled.length === 0 ? (
            <div className="px-4">
              <div className="rounded-xl border border-light bg-white p-4">
            ...
            <div className="px-4 grid grid-cols-2 gap-4">
              {enrolled.map((c) => (
                <div
                  key={c.course_id}
                  className="rounded-xl border border-light bg-white overflow-hidden"
                >
                  ...
                  <div className="p-3">
                    ...
                    <div className="mt-1 text-[11px] text-muted">
                      {Math.round(c.progress_pct)}% complete
                    </div>
                    <Link
        ...
        <Section title="Certificates">
          {loading ? (
            <div className="px-4">
              <div className="h-32 rounded-xl bg-white border border-light animate-pulse" />
            </div>
          ) : certificates.length === 0 && provisionalCerts.length === 0 ? (
            <div className="px-4">
              <div className="rounded-xl border border-light bg-white p-4">
                <p className="text-sm text-muted">
                  No certificates yet. Complete your courses and exams to unlock
                  them.
                </p>
              </div>
            </div>
            ...
            <div className="px-4 grid gap-4">
                ...
                >
                  ...
                  <div className="p-4 space-y-2">
                    <div className="text-xs text-muted">
                      Issued: {formatDate(c.issuedAt)} · Cert No:{" "}
                      {c.certificateNo}
                      {typeof c.scorePct === "number"
                        ? ` · Score: ${c.scorePct}%`
                        : ""}
                    </div>
                    <SimpleCertificate
              ...
              {provisionalCerts.map((p) => (
                <div
                  key={`prov-${p.courseId}`}
                  className="rounded-xl border border-dashed border-light bg-white overflow-hidden"
                >
                  ...
                  <div className="p-4 space-y-2">
                    <div className="text-xs text-muted">
                      Provisional (awaiting issuance) · Passed on{" "}
                      {formatDate(p.issuedAt)}
                      {typeof p.scorePct === "number"
                        ? ` · Score: ${p.scorePct}%`
                        : ""}
                    </div>
                    <SimpleCertificate
            ...
            <div className="px-4 grid gap-4">
              {Object.entries(quizByCourse).map(([courseId, rows]) => {
                ...
                return (
                  <div
                    key={courseId}
                    className="rounded-xl border border-light bg-white p-4"
                  >
